...
try:
    # Safely pulling your brand new, correct keys directly from st.secrets
...
if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
    ...
    # Process historical turns safely and strip old API tracking tokens
    for m in st.session_state.messages[:-1]:
        role_label = "user" if m.get("role") == "user" else "model"
        raw_msg = str(m.get("content", "")).strip()
        
        if not raw_msg or "models/" in raw_msg or "API Execution Failure" in raw_msg:
            continue
