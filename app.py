...
# 3. ADVANCED CSS (Hiding UI & Styling Professional Logo)
st.markdown("""
    <style>
        ...
        with st.form("login_form"):
            ...
            if submit_login:
                try:
                    ...
                    # Double-Check Response to prevent "Ghost" error messages
                    if auth_res.user:
                        ...
                        time.sleep(0.8) # Brief pause for DB handshake
                        st.rerun()
                    else:
            ...
            if submit_signup:
                if new_pass != confirm_pass:
                    ...
                    try:
                        ...
                        st.success("Verification link sent! Check your email inbox.")
                    except Exception as e:
                        if "already" in str(e).lower(): st.error("Email already exists. Log in instead.")
                        else: st.error(f"Error: {e}")
    st.stop()

# 5. POST-LOGIN SYNC & TOAST
...
if prompt := st.chat_input("Message Feemo AI..."):
    ...
    try:
        ...
        st.error("Lost connection to Groq API.")
