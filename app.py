...
# 3. ADVANCED CSS (Hiding UI + Logo Styling)
st.markdown("""
    <style>
    ...
    .logo-symbol { color: #c9a84c; font-size: 40px; margin-right: 10px; }
...
if not st.session_state.authenticated:
    ...
    with tab2:
        with st.form("signup_form"):
            ...
            if submit_signup:
                if new_pass != confirm_pass: st.warning("Passwords mismatch.")
...
# 5. WELCOME POPUP (Temporary 2-second Toast)
...
# --- 8. MAIN LOGO AREA (REPLACES WELCOME TEXT) ---
st.markdown("<div class='logo-container'><span class='logo-symbol'>✦</span><h1 class='logo-text'>FEEMO AI</h1></div>", unsafe_allow_html=True)
...
if prompt := st.chat_input("Message Feemo AI..."):
    ...
    try:
        ...
        payload = {
            ...
            "messages": [{"role": "system", "content": f"You are Feemo AI. Helper to {st.session_state.first_name}"}] + st.session_state.messages
        }
