...
# 3. REFINED GEMINI-STYLE CSS
st.markdown("""
    <style>
    /* Hide top bar clutter but NOT the sidebar toggle */
    footer, .stAppToolbar {visibility: hidden !important;}
    ...
    .stForm { border: 1px solid #2d2d2d !important; padding: 25px !important; border-radius: 15px !important; background-color: #171717; }
    </style>
...
# --- 6. SIDEBAR DEFINITION (No Condition Gate) ---
with st.sidebar:
...
if not st.session_state.authenticated:
    ...
    with t2:
        with st.form("r_form"):
            st.text_input("Name", key="n_name")
            st.text_input("Email", key="n_email")
            st.text_input("Password", type="password", key="n_pass")
            if st.form_submit_button("REGISTER", use_container_width=True):
                try:
                    supabase.auth.sign_up({"email": st.session_state.n_email, "password": st.session_state.n_pass, "options": {"data": {"first_name": st.session_state.n_name}}})
    ...
    if prompt := st.chat_input("Ask Feemo..."):
        ...
        try:
            ...
            if 'pdf_text' in locals() and pdf_text: sys_msg += f"\n\nContext: {pdf_text[:7000]}"
